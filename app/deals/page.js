          ...
          <div style={{ background: 'rgba(255,92,58,0.08)', border: '0.5px solid rgba(255,92,58,0.2)', borderRadius: 16, padding: '16px 20px', marginBottom: 28 }}>
            <div className="banner-row">
              ...
              <div className="banner-btns">
                <Link href="/login" style={{ background: 'rgba(255,255,255,0.1)', color: 'white', border: '0.5px solid rgba(255,255,255,0.2)', padding: '10px 20px', borderRadius: 100, fontSize: 13, textDecoration: 'none', fontFamily: "'DM Sans', sans-serif", display: 'inline-block', textAlign: 'center' }}>
                  Sign in
                </Link>
                <Link href="/pricing" style={{ background: '#FF5C3A', color: 'white', padding: '10px 20px', borderRadius: 100, fontSize: 13, fontWeight: 500, fontFamily: "'DM Sans', sans-serif", textDecoration: 'none', display: 'inline-block', textAlign: 'center', whiteSpace: 'nowrap' }}>
                  Upgrade · ₹999/mo
                </Link>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="deals-grid">
            {sorted.map((deal, i) => {
              ...
              return (
                <div key={deal.id} className="deal-card" style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', border: '0.5px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.07)', backdropFilter: 'blur(16px)', cursor: isLocked ? 'default' : 'pointer' }}>

                  {/* Image */}
                  <div style={{ height: 180, position: 'relative', overflow: 'hidden', background: '#1a1a2a' }}>
                    {deal.image_url && (
                      <img src={deal.image_url} alt={deal.destination} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: isLocked ? 'blur(8px)' : 'none', transform: 'scale(1.05)' }} />
                    )}
