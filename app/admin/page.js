...
function parseGoogleFlightsUrl(url) {
  ...
  try {
    ...
    if (origins.length > 0) {
      ...
      const info = AIRPORT_MAP[origins[0]]
      if (info) { result.origin_city = info.city; result.autoFilled.push('origin_code','origin_city') }
    }
    if (dests.length > 0) {
      result.dest_code = dests[0]
      const info = AIRPORT_MAP[dests[0]]
      if (info) {
        result.destination = info.city; result.country = info.country
        result.region = info.region || 'South East Asia'
        result.autoFilled.push('dest_code','destination','country','region')
      }
    }
  } catch(e) {}
  return result
...
const EMPTY_FORM = {
  ...
  expires_hours: '48', is_free_preview: false
}
...
export default function AdminPage() {
  ...
  function handleFlightUrlChange(e) {
    ...
    setParseMsg(`✅ Auto-filled ${parsed.autoFilled.length} fields from URL.`)
  }
  ...
  async function handleSubmit(e) {
    ...
    const dealData = {
      destination: form.destination, country: form.country, region: form.region,
      origin_city: form.origin_city, origin_code: form.origin_code,
      dest_code: form.dest_code.toUpperCase(), airline: form.airline,
      cabin_class: form.cabin_class, deal_price: parseInt(form.deal_price),
      regular_price: parseInt(form.regular_price), savings_pct: parseInt(form.savings_pct),
      stops: parseInt(form.stops), travel_dates: form.travel_dates,
      booking_url: form.booking_url, image_url: form.image_url,
      description: form.description, is_active: true, expires_at: expiresAt,
    ...
    if (error) {
      ...
      setMessage(editId ? '✅ Deal updated!' : '✅ Deal added!')
      setForm(EMPTY_FORM); setFlightUrl(''); setAutoFilled([]); setParseMsg(''); setEditId(null)
      fetchDeals(); setTab('manage')
    }
    setSaving(false)
  }
  ...
  // ── KEY FUNCTION: toggle free preview ──
  async function toggleFreePreview(deal) {
    ...
    if (turningOn && currentFreeCount >= 3) {
      setMessage('⚠️ Max 3 free preview deals allowed. Remove one first.')
      setTimeout(() => setMessage(''), 3000)
      return
    }
    await supabase.from('deals').update({ is_free_preview: !deal.is_free_preview }).eq('id', deal.id)
    fetchDeals()
  }
  ...
  function editDeal(deal) {
    setForm({
      destination: deal.destination || '', country: deal.country || '',
      region: deal.region || 'South East Asia', origin_city: deal.origin_city || 'Delhi',
      origin_code: deal.origin_code || 'DEL', dest_code: deal.dest_code || '',
      airline: deal.airline || '', cabin_class: deal.cabin_class || 'Economy',
      deal_price: deal.deal_price?.toString() || '', regular_price: deal.regular_price?.toString() || '',
      savings_pct: deal.savings_pct?.toString() || '', stops: deal.stops?.toString() || '0',
      travel_dates: deal.travel_dates || '', booking_url: deal.booking_url || '',
      image_url: deal.image_url || '', description: deal.description || '',
      expires_hours: '48', is_free_preview: deal.is_free_preview || false
    })
    setFlightUrl(deal.booking_url || ''); setAutoFilled([]); setParseMsg('')
    setEditId(deal.id); setTab('add'); window.scrollTo(0, 0)
  }

  const isAutoFilled = (f) => autoFilled.includes(f)
  const autoTag = { fontSize: 10, color: '#4CAF50', fontWeight: 600, marginLeft: 6 }
  const freePreviewCount = deals.filter(d => d.is_free_preview).length
  ...
  const s = {
    ...
    card:   { background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: '16px 20px', marginBottom: 12 },
      ...
      <div style={s.wrap}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 32 }}>
          {[
            ...
            ['Free preview', `${freePreviewCount}/3`],
            ['Pro only', deals.filter(d => d.is_active && !d.is_free_preview).length],
          ].map(([label, val]) => (
          ...
          <form onSubmit={handleSubmit} style={{ background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 28 }}>
            ...
            <div style={{ background: 'rgba(255,92,58,0.06)', border: '1px solid rgba(255,92,58,0.25)', borderRadius: 16, padding: '20px 20px 4px', marginBottom: 28 }}>
              ...
              <p style={{ fontSize: 13, color: 'rgba(255,245,236,0.45)', marginBottom: 14, lineHeight: 1.6 }}>
                Go to <strong style={{ color: 'rgba(255,245,236,0.7)' }}>google.com/travel/flights</strong>, find the deal, copy the URL and paste below.
              </p>
            ...
            <div style={s.grid2}>
              <div><label style={s.label}>Destination city * {isAutoFilled('destination') && <span style={autoTag}>✓ auto-filled</span>}</label>
                <input name="destination" value={form.destination} onChange={handleFormChange} style={isAutoFilled('destination') ? s.inputAuto : s.input} placeholder="e.g. Tokyo" required /></div>
              <div><label style={s.label}>Country code * {isAutoFilled('country') && <span style={autoTag}>✓ auto-filled</span>}</label>
                <input name="country" value={form.country} onChange={handleFormChange} style={isAutoFilled('country') ? s.inputAuto : s.input} placeholder="e.g. JP" required /></div>
            </div>
            <div style={s.grid2}>
              <div><label style={s.label}>Region * {isAutoFilled('region') && <span style={autoTag}>✓ auto-filled</span>}</label>
                <select name="region" value={form.region} onChange={handleFormChange} style={isAutoFilled('region') ? s.selectAuto : s.select}>{REGIONS.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
              <div><label style={s.label}>Origin city * {isAutoFilled('origin_city') && <span style={autoTag}>✓ auto-filled</span>}</label>
                <select name="origin_city" value={form.origin_city} onChange={handleFormChange} style={isAutoFilled('origin_city') ? s.selectAuto : s.select}>{CITIES.map(c => <option key={c} value={c}>{c} ({CITY_CODES[c]})</option>)}</select></div>
            </div>
            <div style={s.grid3}>
              <div><label style={s.label}>Dest. airport code * {isAutoFilled('dest_code') && <span style={autoTag}>✓ auto-filled</span>}</label>
                <input name="dest_code" value={form.dest_code} onChange={handleFormChange} style={isAutoFilled('dest_code') ? s.inputAuto : s.input} placeholder="e.g. NRT" required /></div>
              <div><label style={s.label}>Airline *</label>
                <input name="airline" value={form.airline} onChange={handleFormChange} style={s.input} placeholder="e.g. IndiGo" required /></div>
              <div><label style={s.label}>Cabin class *</label>
                <select name="cabin_class" value={form.cabin_class} onChange={handleFormChange} style={s.select}>{CLASSES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
            </div>
            <div style={s.grid3}>
              <div><label style={s.label}>Deal price (₹) *</label>
                <input name="deal_price" type="number" value={form.deal_price} onChange={handleFormChange} style={s.input} placeholder="e.g. 15850" required /></div>
              <div><label style={s.label}>Regular price (₹) *</label>
                <input name="regular_price" type="number" value={form.regular_price} onChange={handleFormChange} style={s.input} placeholder="e.g. 35000" required /></div>
              <div><label style={s.label}>Savings % (auto-calculated)</label>
                <input name="savings_pct" type="number" value={form.savings_pct} onChange={handleFormChange} style={{ ...s.input, background: 'rgba(76,175,80,0.08)', borderColor: 'rgba(76,175,80,0.2)' }} placeholder="Auto" /></div>
            </div>
            <div style={s.grid3}>
              <div><label style={s.label}>Stops</label>
                <select name="stops" value={form.stops} onChange={handleFormChange} style={s.select}><option value="0">Non-stop</option><option value="1">1 stop</option></select></div>
              <div><label style={s.label}>Travel dates {isAutoFilled('travel_dates') && <span style={autoTag}>✓ auto-filled</span>}</label>
                <input name="travel_dates" value={form.travel_dates} onChange={handleFormChange} style={isAutoFilled('travel_dates') ? s.inputAuto : s.input} placeholder="e.g. Aug–Sept 2026" /></div>
              <div><label style={s.label}>Expires in (hours)</label>
                <select name="expires_hours" value={form.expires_hours} onChange={handleFormChange} style={s.select}>
                  ...
                  <option value="72">72 hours</option><option value="96">96 hours</option><option value="168">7 days</option>
                </select></div>
            </div>
            <div><label style={s.label}>Google Flights URL</label>
              <input name="booking_url" value={form.booking_url} onChange={handleFormChange} style={s.input} placeholder="https://www.google.com/travel/flights/..." /></div>
            <div><label style={s.label}>Image URL (Unsplash)</label>
              <input name="image_url" value={form.image_url} onChange={handleFormChange} style={s.input} placeholder="https://images.unsplash.com/..." /></div>
            <div><label style={s.label}>Description</label>
              <input name="description" value={form.description} onChange={handleFormChange} style={s.input} placeholder="e.g. Non-stop deal to Tokyo — cherry blossom season." /></div>

            {/* Free preview toggle in form */}
            <div style={{ background: 'rgba(76,175,80,0.06)', border: '1px solid rgba(76,175,80,0.2)', borderRadius: 14, padding: '16px 20px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: 'rgba(255,245,236,0.45)' }}>Free users will see this deal without subscribing. Max 3 allowed ({freePreviewCount}/3 used).</div>
              </div>
        ...
        {tab === 'manage' && (
          <div>
            ...
            <div style={{ background: 'rgba(76,175,80,0.07)', border: '0.5px solid rgba(76,175,80,0.25)', borderRadius: 14, padding: '14px 20px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <span style={{ fontSize: 14, fontWeight: 600 }}>🆓 Free Preview slots: </span>
                <span style={{ fontSize: 14, color: freePreviewCount >= 3 ? '#FF8060' : '#4CAF50', fontWeight: 700 }}>{freePreviewCount}/3 used</span>
              </div>
              <span style={{ fontSize: 12, color: 'rgba(255,245,236,0.45)' }}>Toggle the FREE button on any deal to control what free users see</span>
            </div>

            {loading ? (
              <div style={{ textAlign: 'center', padding: 40, color: 'rgba(255,245,236,0.3)' }}>Loading deals...</div>
            ) : deals.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 40, color: 'rgba(255,245,236,0.3)' }}>No deals yet.</div>
            ) : (
              deals.map(deal => (
                <div key={deal.id} style={{ ...s.card, opacity: deal.is_active ? 1 : 0.5, border: deal.is_free_preview ? '1px solid rgba(76,175,80,0.4)' : '0.5px solid rgba(255,255,255,0.1)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', width: '100%' }}>
                    ...
                    {/* Image */}
                    {deal.image_url && (
                      <img src={deal.image_url} alt={deal.destination} style={{ width: 56, height: 56, borderRadius: 10, objectFit: 'cover', flexShrink: 0 }} />
                    )}
                    ...
                    <div style={{ flex: 1, minWidth: 160 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                        ...
                        {deal.is_free_preview && (
                          <span style={{ fontSize: 10, fontWeight: 700, background: 'rgba(76,175,80,0.2)', color: '#4CAF50', border: '0.5px solid rgba(76,175,80,0.4)', padding: '2px 8px', borderRadius: 100, letterSpacing: 0.5 }}>FREE PREVIEW</span>
                        )}
                      </div>
                      <div style={{ fontSize: 12, color: 'rgba(255,245,236,0.5)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
                      ...
                      {/* ── FREE PREVIEW TOGGLE ── */}
                      <button
                        onClick={() => toggleFreePreview(deal)}
                        title={deal.is_free_preview ? 'Remove from free preview' : 'Set as free preview'}
                        style={{
                          fontSize: 12, padding: '6px 14px', borderRadius: 100, border: 'none', cursor: 'pointer',
                          fontFamily: "'DM Sans', sans-serif", fontWeight: 600,
                          background: deal.is_free_preview ? 'rgba(76,175,80,0.2)' : 'rgba(255,255,255,0.07)',
                          color: deal.is_free_preview ? '#4CAF50' : 'rgba(255,245,236,0.4)',
                          boxShadow: deal.is_free_preview ? '0 0 0 1px rgba(76,175,80,0.4)' : 'none',
                        }}>
